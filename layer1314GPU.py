...
if main_phase == 'not_eval':
    for phase in ['train', 'val']:
        ...
        if phase == 'val':
            ...
            torch.save(model, "/root/VOC12_After_b14/TrainBatch3TensorsGPUColor/big_lr/model")

main_phase = 'eval'
if main_phase == 'eval':
    outputs = []
    model = torch.load("/root/VOC12_After_b14/TrainBatch3TensorsGPUColor/big_lr/model")
    model.eval()
    ...
    for i_iter in range(BATCHES * 3):
        ...
        if (i_iter + 1) %3 == 0:
            ...
            torch.save(output, "/root/VOC12_After_b14/TrainBatch3TensorsGPUColor/big_lr/predictions" + str(j) + ".pth")
            outputs = []
